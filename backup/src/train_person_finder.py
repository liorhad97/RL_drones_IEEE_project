...
"""
Training Script for Person Finder Drone

This script demonstrates training and testing a reinforcement learning
agent for person finding missions with drones. It builds on the existing
drone RL architecture and adds the capability to find persons based on
...
# Import our custom environment wrapper
from person_finder_env import PersonFinderEnv
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='Person Finder Drone Training')
    ...
    parser.add_argument('--use_lidar', action='store_true', default=True,
                       help='Use lidar for state abstraction')
    ...
    parser.add_argument('--model_path', type=str, default='models/person_finder_model',
...
def create_environment(args):
    """Create and configure the person finder environment."""
...
def setup_callbacks(args):
    ...
    # Logging callback
    log_dir = os.path.join('logs', 'person_finder')
    ensure_dir(log_dir)
...
def test(env, args):
    """Test the trained person finder agent."""
    # Load model
    try:
        model = SAC.load(args.model_path, env=env)
    ...
    for episode in range(episodes):
        ...
        # Handle different reset() return types (gym vs gymnasium)
        ...
        while not done and step < max_steps:
            ...
            if isinstance(info, tuple):  # Handle different step() return types
                info = info[0]
...
def main():
    ...
    # Setup logging
    logger = setup_logging(log_file='logs/person_finder.log')
    logger.info("Starting Person Finder Drone")
